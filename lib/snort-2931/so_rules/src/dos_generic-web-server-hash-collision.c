...
int rule20825eval(void *p) {
   ...
   DEBUG_SO(printf("rule20825eval enter\n");)
...
//      return RULE_NOMATCH;
   ...
   DEBUG_SO(printf("We have %d bytes of data\n", end_of_payload - cursor_normal);)
   ...
   while(cursor_normal < end_of_payload) {
      DEBUG_SO(printf("%c", *cursor_normal);)

      if(*cursor_normal == '=') {
         paramcount++;
         DEBUG_SO(printf("Found '=' (%d)\n", paramcount);)
